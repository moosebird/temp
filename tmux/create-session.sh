#!/usr/bin/env bash
CONFIG_FILE_NAME="create-session.conf"
CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/tmux"
CONFIG_FILE="$CONFIG_DIR/$CONFIG_FILE_NAME"
PANE_CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/dotfiles"
PANE_CACHE_FILE="$PANE_CACHE_DIR/create-session-panes.cache"

# based on: https://github.com/ThePrimeagen/tmux-sessionizer/blob/master/tmux-sessionizer
# config file example
# ------------------------
# # file: ~/.config/tmux/create-session.conf
...
if [[ $TS_LOG != "true" ]]; then
    if [[ -z $TS_LOG_FILE ]]; then
        TS_LOG_FILE="$HOME/.local/share/dotfiles/create-session.logs"
    fi
...
find_dirs() {
    ...
    for entry in "${TS_SEARCH_PATHS[@]}"; do
		...
		if [[ -d "$path" ]]; then
			# if depth is zero then include just the path itself
			if [[ "$depth" != "0" ]]; then
